...
const {
  sendEmailAccountblocked,
  sendEmailStatusUserAdmin,
...
const authenticateUser = async (data) => {
  try {
    ...
    if (!fetchedUser) {
      throw Error(
        "No encontramos una cuenta vinculada con ese correo electrónico. ¿Estás registrado?"
      );
    }
    ...
    if (!fetchedUser.verified) {
      ...
      await createdLog({
        usuario: fetchedUser._id,
        ip,
        navegador,
        tipo: "1",
        accion,
        descripcion:
          "Hubo un inicio de sesión fallido debido que la cuenta no ha sido verificada",
      });
      throw Error(
        "El correo electrónico aún no se ha verificado. Se ha enviado un código  a tu correo, por favor comprueba tu bandeja de entrada."
      );
    }
    ...
    if (fetchedUser.accountStatus === "bloqueda") {
      ...
      await createdLog({
        ...
        accion,
        descripcion:
          "La cuenta ha sido bloqueada temporalmente debido a varios intentos al iniciar sesión",
      });
      throw Error(
        `La cuenta ha sido bloqueada temporalmente. Comprueba tu bandeja de entrada.`
      );
    }
    ...
    if (!passwordMatch) {
      ...
      await createdLog({
        ...
        accion,
        descripcion:
          "Hubo un inicio de sesión fallido debido que la contraseña es incorrecta",
      });
    ...
    await createdLog({
      ...
      tipo: "1",
      accion,
      descripcion: "Hubo un inicio de sesión correctamente",
    });
...
const updateUser = async (userId, updateData, ip, navegador) => {
  try {
    ...
    await createdLog({
      ...
      tipo: "3",
      accion,
      descripcion: "Algunos campos fueron actualizados correctamente",
    });
    ...
    await createdLog({
      ...
      tipo: "3",
      accion,
      descripcion: "Hubo un error al intentar actualizar algunos campos",
    });
    throw new Error(error.message);
  }
};
...
const authenticateUserMovil = async (data) => {
  try {
    const fecha = moment().add(30, "days").endOf("day").toDate();
    const { email, password, ip, navegador } = data;
...
const getUserByIdByEmail = async (email) => {
  try {
