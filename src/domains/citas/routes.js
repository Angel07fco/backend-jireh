...
router.get("/validation/:fecha/:mascota", async (req, res) => {
...
router.post("/newcita", async (req, res) => {
  let { usuario, mascota, servicio, medico, fecha, hora, comentarios } =
    req.body;
  try {
...
router.get("/citas/:medico/:fecha", async (req, res) => {
...
router.delete("/cancelar/:citaId", async (req, res) => {
...
router.put("/actualizar/:citaId", async (req, res) => {
  const { citaId } = req.params;
...
router.put("/feedback/:citaId", async (req, res) => {
  const { citaId } = req.params;
  const updateData = req.body;

  try {
    const updatedCita = await updateCitaFeedback(citaId, updateData);
    res.status(200).json({ mensaje: updatedCita });
  } catch (error) {
    res.status(400).send(error.message);
  }
...
router.get("/alexa/:usuario", async (req, res) => {
  ...
  try {
    ...
    if (!citasHoy.length) {
      return res
        .status(404)
        .send("No se encontraron citas futuras para hoy con estado 'proxima'.");
    }

    // Ordenar las citas de hoy por hora
    const citasOrdenadas = citasHoy.sort(
      (a, b) => moment(a.hora, "HH:mm").unix() - moment(b.hora, "HH:mm").unix()
    );
    const citaMasProxima = citasOrdenadas[0];

    const fechaFormateada = moment(citaMasProxima.fecha, "DD-MM-YYYY").format(
      "D [de] MMMM"
    );
    const horaFormateada = moment(citaMasProxima.hora, "HH:mm")
      .format("h [de la] A")
      .replace("AM", "mañana")
      .replace("PM", "tarde");
...
router.get("/alexa/info/:usuario/:fecha/:hora", async (req, res) => {
  ...
  try {
    ...
    if (citaInfo.length) {
      return res
        .status(404)
        .send("No se encontró una cita para esa fecha y hora.");
    }

    const fechaFormateada = moment(citaInfo.fecha, "DD-MM-YYYY").format(
      "D [de] MMMM"
    );
    const horaFormateada = moment(citaInfo.hora, "HH:mm")
      .format("h [de la] A")
      .replace("AM", "mañana")
      .replace("PM", "tarde");
...
router.delete(
  "/alexa/cancelar-cita/:usuario/:fecha/:hora",
  async (req, res) => {
    const { usuario, fecha, hora } = req.params;
    try {
      const citaInfo = await getCitaByUserIdFechaHora(usuario, fecha, hora);
...
router.get("/filtrar/:fecha", async (req, res) => {
  ...
  try {
    ...
    if (!citas.length) {
      return res
        .status(404)
        .send("No se encontraron citas para la fecha especificada.");
    }
    res.status(200).json(citas);
  } catch (error) {
    res.status(400).send(error.message);
